...
import {
  createFactura as createFacturaDB,
  updateFacturaStatus as updateFacturaStatusDB,
...
export async function createFactura(
  ...
  try {
    ...
    return {
      success: false,
      message:
        error instanceof Error ? error.message : "Failed to create factura",
    };
  }
}
...
export async function updateFacturaStatus(
  ...
  try {
    ...
    await updateFacturaStatusDB(
      facturaId,
      estado,
      getOptionalString("fechaPago"),
      getOptionalString("metodoPago"),
      formData.get("montoPagado")
        ? Number(formData.get("montoPagado"))
        : undefined,
      getOptionalString("comentarios")
    );
    ...
    return {
      ...
      message:
        error instanceof Error
          ? error.message
          : "Failed to update factura status",
    };
  }
}
