...
export default function RegistrarFacturaPage() {
  ...
  const [autorizacionesAprobadas, setAutorizacionesAprobadas] = useState<Autorizacion[]>([])
  ...
  useEffect(() => {
    const cargarAutorizacionesAprobadas = async () => {
      ...
      try {
        ...
        const autorizaciones = await dbService.getAutorizacionesByEstado("aprobada")
        setAutorizacionesAprobadas(autorizaciones)
      } catch (error) {
  ...
  useEffect(() => {
    const cargarAfiliado = async () => {
      if (autorizacionSeleccionada) {
        try {
          const dbService = DbService.getInstance()
          const afiliadoData = await dbService.getAfiliadoById(autorizacionSeleccionada.afiliadoId)
  ...
  const handleSelectAutorizacion = (autorizacionId: string) => {
    const autorizacion = autorizacionesAprobadas.find((a) => a.id === autorizacionId) || null
    ...
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-3xl font-bold text-gray-800">Registrar Factura</h1>
        </div>
      </div>
          ...
          <div className="grid gap-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Seleccione una autorización médica aprobada para facturar</CardDescription>
              </CardHeader>
              <CardContent>
                {loading ? (
                  ...
                  <Alert>
                    ...
                    <AlertDescription>
                      No se encontraron autorizaciones médicas aprobadas para facturar.
                    </AlertDescription>
                  </Alert>
                ) : (
                  <div className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <Select onValueChange={(value) => handleSelectAutorizacion(value)}>
                        <SelectTrigger>
                          <SelectValue placeholder="Seleccione una autorización" />
                        </SelectTrigger>
                        <SelectContent>
                          {autorizacionesAprobadas.map((autorizacion) => (
                            <SelectItem key={autorizacion.id} value={autorizacion.id}>
                              {autorizacion.numeroAutorizacion} - {autorizacion.tipoServicio}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                    ...
                    {autorizacionSeleccionada && (
                      <div className="rounded-md border p-4 mt-4">
                        <div className="flex justify-between items-start">
                          <div>
                            <h3 className="font-medium">Autorización: {autorizacionSeleccionada.numeroAutorizacion}</h3>
                            <p className="text-sm text-gray-500">Servicio: {autorizacionSeleccionada.tipoServicio}</p>
                          </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <p className="text-sm">{autorizacionSeleccionada.prestador}</p>
                          </div>
                          <div>
                            <p className="text-sm font-medium text-gray-500">Médico Tratante</p>
                            <p className="text-sm">{autorizacionSeleccionada.medicoTratante || "No especificado"}</p>
                          </div>
                          <div>
                            <p className="text-sm font-medium text-gray-500">Fecha de Servicio</p>
                            <p className="text-sm">{autorizacionSeleccionada.fechaServicio}</p>
                          </div>
                          <div>
                            <p className="text-sm font-medium text-gray-500">Monto Estimado</p>
                            <p className="text-sm">
                              RD${" "}
                              {autorizacionSeleccionada.montoEstimado.toLocaleString("es-DO", {
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Ingrese los datos generales de la factura</CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Adjunte la documentación requerida para la factura</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <div>
                  ...
                  <div>
                    <Label htmlFor="documentosSoporte">Documentos de Soporte</Label>
                    <Input id="documentosSoporte" type="file" multiple className="mt-1" />
                  </div>
                  <div className="space-y-2">
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {formData.montoTotal.toLocaleString("es-DO", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}?
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button onClick={confirmSubmit} className="bg-purple-600 hover:bg-purple-700" disabled={loading}>
              {loading ? (
                <>
