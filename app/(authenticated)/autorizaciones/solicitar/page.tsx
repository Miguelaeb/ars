...
export default function SolicitarAutorizacionPage() {
  ...
  const [urgencia, setUrgencia] = useState<"normal" | "urgente" | "emergencia">(
    "normal"
  );
  ...
  const [autorizacionCreada, setAutorizacionCreada] =
  ...
  const buscarAfiliado = async () => {
    ...
    try {
      const res = await fetch("/api/afiliados/buscar", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ cedula: cedulaAfiliado }),
      });
      ...
      if (!res.ok) {
        toast({
          ...
          description:
            "No se encontró ningún afiliado con la cédula proporcionada",
          variant: "destructive",
        });
        setAfiliado(null);
        return;
      ...
      const afiliadoEncontrado = await res.json();
  ...
  const confirmSubmit = async () => {
    ...
    formData.append(
      ...
      cobertura.requiereAutorizacion.toString()
    );

    try {
      const result = await createAutorizacion({
        ...
        urgencia,
        porcentajeCobertura: cobertura.porcentaje,
        copago: cobertura.copago,
        montoMaximo: cobertura.montoMaximo,
        requiereAutorizacion: cobertura.requiereAutorizacion,
        estado: cobertura.estado,
        comentarios: "",
        documentos: [], // si deseas procesar archivos luego puedes ajustarlo
      });
      ...
      toast({
        title: "Error",
        description:
          "Ocurrió un error al procesar la solicitud de autorización",
        variant: "destructive",
      });
    }
  };
    ...
    <div className="flex flex-col gap-5">
      ...
      {formSubmitted ? (
        <Alert className="bg-green-50 border-green-200">
          ...
          <AlertDescription>
            La solicitud de autorización ha sido registrada correctamente en el
            sistema.
            {autorizacionCreada && (
              <p className="mt-2 font-medium">
                Número de autorización: {autorizacionCreada.numeroAutorizacion}
              </p>
            )}
            <div className="mt-4">
              <Button asChild>
                <Link href="/autorizaciones">
                  Ver listado de autorizaciones
                </Link>
              </Button>
            </div>
          </AlertDescription>
          ...
          <div className="grid gap-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Busque y seleccione el afiliado para la autorización
                </CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Seleccione el servicio y valide la cobertura
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="clinica-san-jose">
                          Clínica San José
                        </SelectItem>
                        <SelectItem value="centro-medico-nacional">
                          Centro Médico Nacional
                        </SelectItem>
                        <SelectItem value="laboratorio-clinico">
                          Laboratorio Clínico Moderno
                        </SelectItem>
                        <SelectItem value="centro-diagnostico">
                  ...
                  <div className="space-y-2">
                    ...
                    <RadioGroup
                      ...
                      onValueChange={(value) =>
                        setUrgencia(
                          value as "normal" | "urgente" | "emergencia"
                        )
                      }
                  ...
                  <div className="mt-6 rounded-md border p-4">
                      ...
                      {cobertura.estado === "aprobado" ? (
                        <Alert className="bg-green-50 border-green-200 p-2">
                          ...
                          <AlertDescription className="text-green-600 text-sm">
                            Este servicio está cubierto y no requiere
                            autorización previa.
                          </AlertDescription>
                        </Alert>
                      ) : (
                        <Alert className="bg-yellow-50 border-yellow-200 p-2">
                          ...
                          <AlertDescription className="text-yellow-600 text-sm">
                            Este servicio requiere autorización previa para ser
                            cubierto.
                          </AlertDescription>
                        </Alert>
                      )}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-bold">
                          RD$ {cobertura.montoMaximo.toLocaleString()}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm font-medium text-gray-500">
                        ...
                        <p className="text-sm font-bold">
                          {cobertura.requiereAutorizacion ? "Sí" : "No"}
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Adjunte la documentación requerida para la autorización
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div>
                    <Label htmlFor="resultadosPrevios">
                      Resultados de Estudios Previos
                    </Label>
                    <Input
                      id="resultadosPrevios"
                      name="resultadosPrevios"
                      type="file"
                      className="mt-1"
                    />
                  </div>
      ...
      <Dialog open={showConfirmation} onOpenChange={setShowConfirmation}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ¿Está seguro que desea enviar esta solicitud de autorización?
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
